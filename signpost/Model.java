 ...
 * Didscuss the part of initializing part with Annie Pang.
 */
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        Place[] solnNumToPlace = new Place[last + 1];
        _board = new Sq[_width][_height];
        for (int i = 0; i < _width; i = i + 1) {
            for (int j = 0; j < _height; j = j + 1) {
                if (_solution[i][j] == 1) {
                    _board[i][j] = new Sq(i, j, 1, true,
                            arrowDirection(i, j), 0);
                } else if (_solution[i][j] == last) {
                    _board[i][j] = new Sq(i, j, last, true,
                            arrowDirection(i, j), 0);
                } else {
                    _board[i][j] = new Sq(i, j, 0, false,
                            arrowDirection(i, j), -1);
                }
                solnNumToPlace[_solution[i][j]] = Place.pl(i, j);
            }
        }
        for (Sq[] col : _board) {
            for (Sq sq : col) {
                ...
                for (int x = 0; x < _width; x = x + 1) {
                    for (int y = 0; y < _height; y = y + 1) {
                        for (int z = 1; z < 9; z = z + 1) {
                            for (int h = 0;
                                 h < _allSuccessors[x][y][z].size();
                                 h = h + 1) {
                                if (_allSuccessors[x][y][z].get(h)
                                        == Place.pl(i, j)) {
                                    temp.add(Place.pl(x, y));
                                }
                            }
                        }
                    }
                }
                _board[i][j]._predecessors = new PlaceList(temp);
            }
        }
        _unconnected = last - 1;
    }
    ...
    Model(Model model) {
        ...
        for (Sq[] lst : _board) {
            for (Sq y : lst) {
                y._successor = get(model.get(y)._successor);
                y._predecessor = get(model.get(y)._predecessor);
                y._head = get(model.get(y)._head);
            }
        }
    }
    ...
    void init(int width, int height) {
        ...
        _allSuccessors = successorCells(width, height);
    }
     ...
     */
    boolean autoconnect() {
        for (int x1 = 0; x1 < _width; x1 = x1 + 1) {
            for (int y1 = 0; y1 < _height; y1 = y1 + 1) {
                if (_board[x1][y1]._sequenceNum != 0
                        && _board[x1][y1].successor() == null) {
                    for (int x2 = 0; x2 < _width; x2 = x2 + 1) {
                        for (int y2 = 0; y2 < _height; y2 = y2 + 1) {
                            if (_board[x1][y1].connectable(_board[x2][y2])
                                    && _board[x1][y1].sequenceNum() + 1
                                    == _board[x2][y2].sequenceNum()) {
                                _board[x1][y1].connect(_board[x2][y2]);
                                return true;
                            }
                        }
                    }
    ...
    void solve() {
        for (int i = 0; i < _width; i = i + 1) {
            for (int j = 0; j < _height; j = j + 1) {
                ...
                for (int x = 0; x < _width; x = x + 1) {
                    for (int y = 0; y < _height; y = y + 1) {
                        if (_board[i][j].sequenceNum() + 1
                                == _board[x][y].sequenceNum()) {
                            autoconnect();
                            _board[i][j].connect(_board[x][y]);
                        }
                    }
                }
            }
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() != 0 && s1.sequenceNum() == 0) {
                Sq tem = s1;
                releaseGroup(tem.group());
                while (tem != null) {
                    tem._sequenceNum = tem.predecessor().sequenceNum() + 1;
                    tem = tem.successor();
                }
            }
            if (s1.sequenceNum() != 0 && sequenceNum() == 0) {
                ...
                releaseGroup(group());
                while (tema != null) {
                    tema._sequenceNum = tema.successor().sequenceNum() - 1;
                    tema = tema.predecessor();
                }
            }
            Sq point = s1;
            while (point != null) {
                point._head = this.head();
                point = point.successor();
            }
            if (sequenceNum() == 0 && s1.sequenceNum() == 0) {
                this._head._group = joinGroups(this.group(), s1._group);
            }
            return true;
        }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this.predecessor() == null && next.successor() == null) {
                    ...
                    for (Place yy : next.successors()) {
                        get(yy)._group = newGroup();
                    }
                }
            } else {
                int count = 0;
                for (Sq te = this; te != null; te = te.predecessor()) {
                    if (te.hasFixedNum()) {
                        count = 1; break;
                    }
                }
                if (count == 0) {
                    ...
                    if (this.predecessor() != null) {
                        for (Place pa : this.predecessors()) {
                            get(pa)._group = newGroup();
                        }
                    } else {
                        this._group = -1;
                    }
                }
                next._head = next; count = 0;
                for (Sq tpa = next; tpa != null; tpa = tpa.successor()) {
                    if (tpa.hasFixedNum()) {
                        count = 1; break;
                    }
                }
                if (count == 0) {
                    for (Sq p = next; p != null; p = p.successor()) {
                        p._sequenceNum = 0;
                    }
                    if (next._successor == null) {
                        next._group = -1;
                    } else {
                        for (Place pt : next.successors()) {
                            get(pt)._group = newGroup();
                        }
                    }
                }
            }
            for (Sq tem = next; tem != null; tem = tem._successor) {
                tem._head = next;
            }
        }
        @Override
