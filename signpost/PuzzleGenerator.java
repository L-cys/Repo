    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place templace : sq.successors()) {
                    if (sq.connectable(model.get(templace))) {
                        nFound = nFound + 1;
                        found = model.get(templace);
                        if (sq.sequenceNum() != 0
                                && model.get(templace).sequenceNum() != 0) {
                            nFound = 1;
                            found = model.get(templace);
                            break;
                        }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (Place templace : sq.predecessors()) {
                    if (model.get(templace).connectable(sq)) {
                        nFound++;
                        found = model.get(templace);
                        if (sq.sequenceNum() != 0
                                && model.get(templace).sequenceNum() != 0) {
